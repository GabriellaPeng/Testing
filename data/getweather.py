...
from path import station_data

# if we don't have the stations file locally, download it
my_file = Path(station_data+ '/chapter5/stations.txt')
...
allstations = np.genfromtxt(my_file, delimiter=[11,9,10,7,3,31,4,4,6],
...
def getdata(station_name):
    ...
    pw = pw[pw.value != -9999]
    # why use above? why not 'pw[~np.isnan(pw.value)]'
